...
void KeyFrame::Init(v8::Local<v8::Object> exports) {
    ...
    Nan::SetAccessor(tpl->InstanceTemplate(), Nan::New("graphNode").ToLocalChecked(), GetGraphNode);
...
void KeyFrame::GetId(v8::Local<v8::String> property, const Nan::PropertyCallbackInfo<v8::Value>& info) {
    KeyFrame* obj = ObjectWrap::Unwrap<KeyFrame>(info.Holder());
    info.GetReturnValue().Set(obj->self->id_);
}

void KeyFrame::GetGraphNode(v8::Local<v8::String> property, const Nan::PropertyCallbackInfo<v8::Value>& info) {
    KeyFrame* obj = ObjectWrap::Unwrap<KeyFrame>(info.Holder());
    info.GetReturnValue().Set(GraphNode::NewInstance(obj->self->graph_node_));
}
